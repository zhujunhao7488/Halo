  ...
  size_t ht_status(int resize_increase, int just_print) {
    ...
    if (just_print) {
      ...
      if (full_ratio > 0 && full_ratio < CLHT_PERC_FULL_HALVE) {
        // ht_resize_pes(h, 0, 33, dmmanager);
...
class Halo {
 public:
  ...
  bool Update(Pair_t<KEY, VALUE> &p, int *r) {
    ...
    // if (sz.first == -1) return true;
    // auto offset = sz.second;
    // if (!memory_manager_Pool.is_in_allocating(offset / PAGE_SIZE))
    //   if (float(sz.first) / PAGE_SIZE > CLEAN_THRESHOLD &&
    //       PPage_table[offset / PAGE_SIZE].load() != UINT64_MAX - 1) {
    //     PPage_table[offset / PAGE_SIZE] = UINT64_MAX - 1;
    //     reclaim_ppage(offset / PAGE_SIZE);
    //   }
